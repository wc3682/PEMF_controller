...
void setup() {
  ...
  // --- CORRECTED TIMER INITIALIZATION for newer ESP32 Core ---
  Serial.println("Configuring ISR Engine Timer...");
  // Use timer 0. Set a 1MHz base clock (1 tick = 1 microsecond).
  tick_timer = timerBegin(1000000);
  // Attach the ISR function.
  timerAttachInterrupt(tick_timer, onTimerISR);
  // Set the alarm to fire every TICK_INTERVAL_US microseconds and auto-reload.
  timerAlarm(tick_timer, TICK_INTERVAL_US, true, 0);
  // The timer is configured but not started yet. timerStart() will be called in startPEMF().
...
void startPEMF() {
  ...
  portEXIT_CRITICAL(&timerMux);
    ...
    // --- CORRECTED TIMER ENABLE ---
  timerStart(tick_timer);
...
void stopPEMF() {
  ...
  // --- CORRECTED TIMER DISABLE ---
  if (tick_timer != NULL) {
    timerStop(tick_timer);
